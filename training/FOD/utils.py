...
from FOD.Loss import ScaleAndShiftInvariantLoss, CeDepthLoss
from FOD.Custom_augmentation import ToMask

def get_total_paths(path, ext):
    return glob(os.path.join(path, '*'+ext))

def get_splitted_dataset(config, split, dataset_name, path_images, path_depths, path_segmentation=None):
...
def get_losses(config):
    ...
    if type == "full" or type=="depth":
        ...
        elif config['General']['loss_depth'] == 'ce':
            loss_depth = CeDepthLoss()
