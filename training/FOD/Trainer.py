        ...
        for epoch in range(epochs):  # loop over the dataset multiple times
            ...
            for i, batch in enumerate(pbar):
                if self.with_segmentation:
                    (X, Y_depths, Y_segmentations) = batch
                ...
                if self.with_segmentation:
                    ...
                    loss_segmentation = self.loss_segmentation(output_segmentations, Y_segmentations)
                    loss = loss + loss_segmentation
                # get loss
                loss.backward()
                # step optimizer
    ...
    def run_eval(self, val_dataloader):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(pbar):
                ...
                if self.with_segmentation:
                    loss_segmentation = self.loss_segmentation(output_segmentations, Y_segmentations)
                    loss = self.loss_depth(output_depths, Y_depths) + loss_segmentation
                else:
                    loss = self.loss_depth(output_depths, Y_depths)
                val_loss += loss.item()
            ...
            if self.config['wandb']['enable']:
                ...
                if self.with_segmentation:
                    ...
                    self.img_logger(X_1, Y_depths_1, None, output_depths_1, None)
        return val_loss/(i+1)
